  ...
  return (
    <>
      <Link
        href="/painel"
        prefetch={false}
        className="mb-4 inline-flex items-center gap-1.5 text-sm text-tinta-2"
      >
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <div
            key={card.label}
            className="rounded-2xl border border-contorno bg-fundo p-4"
          >
            <p className="text-xs text-tinta-2">{card.label}</p>
            <p className="mt-1 font-bold tabular-nums text-tinta">
              {card.value}
            </p>
            {card.hint && <p className="mt-0.5 text-xs text-tinta-2">{card.hint}</p>}
          </div>
        ))}
      </div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-contorno bg-fundo p-5">
          <h3 className="text-sm font-semibold text-tinta">Família</h3>
          <dl className="mt-3 space-y-2 text-sm">
            <div className="flex justify-between gap-4">
              <dt className="text-tinta-2">Responsável</dt>
              <dd className="text-right text-tinta">
                {athlete.guardian_name ?? "não vinculado"}
              </dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-tinta-2">Tarefas concluídas</dt>
              <dd className="text-right tabular-nums text-tinta">
                {athlete.tasks_done}
              </dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-tinta-2">Tarefas abertas</dt>
              <dd className="text-right tabular-nums text-tinta">
                {athlete.tasks_open}
              </dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-tinta-2">Documentos vencidos</dt>
              <dd
                className={`text-right tabular-nums ${
                  athlete.docs_expired > 0 ? "font-semibold text-red-700" : "text-tinta"
                }`}
              >
            ...
            <div className="flex justify-between gap-4">
              <dt className="text-tinta-2">Moedas</dt>
              <dd className="text-right tabular-nums text-tinta">{athlete.coins}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-tinta-2">Cadastrado em</dt>
              <dd className="text-right text-tinta">
      ...
      {history && history.length > 0 && (
        <section className="mt-4 rounded-2xl border border-contorno bg-fundo p-5">
          <h3 className="text-sm font-semibold text-tinta">
            Histórico de avaliações
          </h3>
          <ul className="mt-3 space-y-3">
            {history.map((entry) => (
              <li key={entry.id} className="border-l-2 border-contorno pl-3">
                ...
                <p className="mt-0.5 text-sm text-tinta-2">{entry.reason}</p>
                {entry.note && (
                  <p className="mt-0.5 text-sm italic text-tinta">“{entry.note}”</p>
                )}
              </li>
            ))}
          </ul>
